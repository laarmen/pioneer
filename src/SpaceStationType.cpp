...
static int _get_stage_durations(lua_State *L, const char *key, int &outNumStages, double **outDurationArray)
{
	...
	std::vector<double> stages = t.GetVector<double>();
	if (stages.size() < 1)
	...
	*outDurationArray = new double[stages.size()];
	std::copy(stages.begin(), stages.end(), *outDurationArray);
...
static int _get_bay_ids(lua_State *L, const char *key, SpaceStationType::TBayGroups &outBayGroups)
{
	...
	std::vector<LuaTable> bayGroups = t.SubVector();
	if (bayGroups.empty()) {
		return luaL_error(L, "Station must have at least 1 group of bays in %s", key);
	}

	for (int i = 0; i < bayGroups.size(); ++i) {
		SpaceStationType::SBayGroup newBay;
		newBay.minShipSize = bayGroups[i].Get<int>(1);
		newBay.maxShipSize = bayGroups[i].Get<int>(2);
		LuaTable group = bayGroups[i].Sub(3);

		if (group.GetLua() == 0) {
			luaL_error(L, "A group is of the form {int, int, table} (%s)", key);
		}

		std::vector<int> bayIDs = group.GetVector<int>();
		if (bayIDs.empty()) {
			return luaL_error(L, "Group must have at least 1 bay %s", key);
		}

		newBay.bayIDs.reserve(bayIDs.size());
		for (std::vector<int>::const_iterator it = bayIDs.begin(); it != bayIDs.end(); it++) {
			if ((*it) < 1) {
				return luaL_error(L, "Valid bay ID ranges start from 1 %s", key);
			}
			newBay.bayIDs.push_back((*it)-1);
		}
		lua_pop(L, 1); // Popping group
		outBayGroups.push_back(newBay);
	}
	lua_pop(L, bayGroups.size()+1); // Popping bayGroups and t
	LUA_DEBUG_END(L, 0);
	return 0;
}
